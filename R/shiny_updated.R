    ...
    sidebarMenu(id = 'sidebarmenu',
                ...
                conditionalPanel("input.sidebarmenu === 'segmentCells'",
                                 ...
                                 sliderInput('shrinkBar', '4. Shrink Threshold',
                                             0, 20, value = 5, step = 1),
                                 sliderInput('sizeFraction', '5. Select size range', min = 0.1, max = 1, step = 0.1, value = 0.1),
                                 radioButtons('scale_input', label = '6. Record scale', choices = c('Y','N'), selected = 'N', inline = T),
                ...
                conditionalPanel("input.sidebarmenu === 'cellId'",
                                 selectInput('id', 'Species',
                                              choices = c('Non algal', 'm1', 'm2', 'm3', 'm4',
                                                          ...
                                                          'snow', 'bluegreen','other'), selected = 0),
                                 radioButtons('rec', 'Record Input', choices = c('Y','N'), selected = 'N', width = '200px', inline = T),
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = 'cellId',
              displayOutput('img2', width = '100%', height = '800px')
      ),
      tabItem(tabName = 'summary',
              fluidRow(
                ...
                column(12,
                       plotOutput('img3', width = '100%', height = '800px')
                       ),
                 column(12,
...
server <- function(input, output, session) {
  ...
  observe({
    app.data$initial_total_blobs_ebimage_single_img
    input$sizeFraction
    input$thresBar
    if(!is.null(app.data$initial_fea_shape)){

      #get range of areas (from EBImage initial_fea_shape) and threshold proportionally
      initial.areas <- isolate(app.data$initial_fea_shape$s.area)
      area.thres <- max(initial.areas) * 0.1#input$sizeFraction

      #which to drop?
      to.rm <- which(initial.areas < area.thres)

      #remove from initial pix set to create new pixset for subsequent analysis
      f <- isolate(app.data$initial_total_blobs_imager) # want to remove from this object
  ...
  output$img1 <- renderDisplay({ #EBImage derived from Imager product
    ...
    input$thresBar
    input$sizeFraction
    app.data$final_total_blobs_imager
  ...
  #render a new plot that shows which cells have been identified - does not seem to be working
